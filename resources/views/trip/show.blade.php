<x-app-layout>
    <x-slot:title>{{ $trip->name . ' - Best tour' }}</x-slot:title>
    ...
    <div x-data="{open: false}" class="pb-5">
        ...
        <div x-show="open" x-cloak x-transition class="sm:mx-8 mx-4 bg-gradient-to-tr from-slate-100 via-gray-200 to-neutral-300 ">
            <form class="px-5 py-3 sm:px-10 sm:py-6" id="book-trip" method="post">
                ...
                @if (!Auth::check())
                    <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
                        ...
                        <div class="text-center">cliqué ici pour <a href="{{ route('login') }}"
                                class="underline font-medium">s'authentifier</a> ou <a href="{{ route('register') }}"
                                class="underline font-medium">créer un nouveau
                                compte</a>.</div>
                    </div>
                @endif
                <div class="grid gap-5 mb-4 sm:grid-cols-3">
                    {{-- Trip date --}}
                    <div>
                        <label for="date_id" class="text-lg font-bold dark:text-white">Selectionnez Départ</label>
                        <select id="date_id" name="date_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            @foreach ($trip->tripDates as $date)
                                <option value="{{ $date->id }}">
                                    {{ 'Départ le ' . $date->date_departure . ' - Retour le ' . $date->date_return }}
                                </option>
                            @endforeach
                        </select>
                        @error('date_id')
                    ...
                    <div>
                        ...
                        <select id="formule" name="formule"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option value="{{ $trip->formule_base }}">
                                {{ $formule_meaning[$trip->formule_base] . ' (' . $trip->formule_base . ')' }}
                            </option>
                        </select>
                        @error('formule')
                            <div class="text-red-800 error">{{ $message }}</div>
    ...
    <script>
        window.onload = function() {
            ...
            async function calculateNewPrice() {
                ...
                try {
                    const response = await axios.post('/api/trip/{{ $trip->slug }}/calculate-price', {
