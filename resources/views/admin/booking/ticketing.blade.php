<x-admin-layout>
                    ...
                    <div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="order_by" value="oldest"
                                {{ request()->order_by == 'oldest' ? 'checked' : '' }}
                                class="sr-only peer">
                            <div
    ...
    @php
        $type = ['AR' => 'Aller retour', 'AS' => 'Aller simple'];
    @endphp
                ...
                @foreach ($bookings as $booking)
                    <tr
                        ...
                        <th scope="row"
                            ...
                            <a class="underline" href="{{ route('admin.booking.show', ['ref' => $booking->ref]) }}">
                                {{ $booking->ref }}
                        ...
                        {{-- type --}}
                        <td class="px-2 py-4">
                            {{ $type[$booking->ticketing->flight_type] }}
                        </td>
                        {{-- date --}}
                        <td class="px-2 py-4">
                            {{ $booking->date_departure }}
                            {{ $booking->date_return ? ' - ' . $booking->date_return : '' }}
                        </td>
                        {{-- compagnie --}}
                        <td class="px-2 py-4">
                            {{ $booking->ticketing->compagnie }}
                        </td>
                        {{-- class --}}
                        <td class="px-2 py-4">
                            {{ $booking->ticketing->class }}
                        </td>
                        {{-- airport depart&return --}}
                        <td class="px-2 py-4">
                            {{ $booking->ticketing->airport_departure . ' - ' . $booking->ticketing->airport_arrived }}
                        </td>
                        {{-- status --}}
                        <td class="px-2 py-4">
                            @if ($booking->status)
                                    ...
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                                Êtes-vous sûr de vouloir archiver la réservation n°
                                                {{ $booking->ref }} ?
                                            </h3>
                                            <form method="post"
